...
-- Table storing applicant information and thresholds
CREATE TABLE dbo.Applicants (
    ApplicantID INT IDENTITY PRIMARY KEY,
    FamilyName NVARCHAR(100) NOT NULL,
    PhoneNumber VARCHAR(20) NOT NULL,
    Address NVARCHAR(200) NOT NULL,
    FamilySize INT NOT NULL CHECK (FamilySize BETWEEN 1 AND 17),
...
INSERT INTO dbo.Applicants (
    FamilyName, PhoneNumber, Address, FamilySize,
    GrossIncomeLimit, MaxBenefitAmount, StandardDeduction,
    EarnedIncome, UnearnedIncome, DaycareExpense, ShelterExpense, UtilityExpense
)
VALUES
    ('Smith', '732-901-1234', '1 Happy Lane', 4, 4086, 835, 184, 2500, 500, 180, 900, 548),
    ('Adams', '732-942-0987', '2 Joyous Drive', 8, 6886, 150, 246, 5400, 1200, 360, 1750, 548),
    ('Jackson', '848-226-8765', '3 Momentous Place', 2, 2686, 459, 177, 1500, 1000, 0, 1000, 29),
    ('Tyler', '732-886-6543', '4 Drake Road', 3, 3386, 658, 177, 3000, 1200, 400, 1800, 548),
    ('Buchanan', '732-994-2345', '5 Hickory Drive', 15, 11786, 282, 246, 10000, 0, 810, 2250, 548),
    ('Arthur', '848-210-9843', '6 Misory Lane', 1, 1986, 250, 177, 9870, 0, 0, 670, 29),
    ('Wilson', '732-290-8764', '7 Smoke Lane', 3, 3386, 658, 177, 1500, 301, 230, 1000, 369),
    ('Hoover', '732-543-2109', '8 Justice Place', 7, 6186, 131, 246, 3434, 3434, 343, 1343, 548),
    ('Truman', '732-994-2210', '9 Yorktown Blvd', 5, 4786, 992, 215, 4500, 1200, 400, 1800, 548),
    ('Johnsohn', '732-998-7654', '10 Branchtown Road', 10, 8286, 188, 246, 10000, 500, 510, 1200, 548),
    ('Nixon', '732-886-4325', '11 Letsgo Place', 5, 4786, 992, 215, 400, 600, 300, 1150, 369),
    ('Carter', '732-990-9990', '12 Bored Town Road', 2, 2686, 459, 177, 2000, 500, 0, 1150, 369),
    ('Biden', '848-998-9876', '13 Icantthink Lane', 13, 10386, 244, 246, 9950, 100, 0, 1800, 548),
    ('Cohen', '732-765-4321', '14 Winya Place', 9, 7586, 169, 246, 6000, 1300, 400, 275, 369);
GO
...
SELECT
    ...
    CASE WHEN tot.TotalIncome <= a.GrossIncomeLimit AND benefit.BenefitAmount > 0 THEN 1 ELSE 0 END AS IsEligible
...
CROSS APPLY (
    SELECT CASE WHEN gross.GrossIncome - a.StandardDeduction - a.DaycareExpense < 0 THEN 0
                ELSE CAST(gross.GrossIncome - a.StandardDeduction - a.DaycareExpense AS DECIMAL(10,2)) END AS NetIncomeBeforeShelter
) nibs
...
CROSS APPLY (
     SELECT CASE WHEN a.MaxBenefitAmount - thirty.ThirtyPercentNetIncome < 0 THEN 0
                 ELSE CAST(a.MaxBenefitAmount - thirty.ThirtyPercentNetIncome AS DECIMAL(10,2)) END AS BenefitAmount
) benefit;
GO
